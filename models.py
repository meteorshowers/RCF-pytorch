...
class RCF(nn.Module):
    ...
    def forward(self, x):
        ...
        so1 = self.score_dsn1(conv1_1_down + conv1_2_down)
        so2 = self.score_dsn2(conv2_1_down + conv2_2_down)
        so3 = self.score_dsn3(conv3_1_down + conv3_2_down + conv3_3_down)
        so4 = self.score_dsn4(conv4_1_down + conv4_2_down + conv4_3_down)
        so5 = self.score_dsn5(conv5_1_down + conv5_2_down + conv5_3_down)
        ...
        upsample5 = torch.nn.functional.conv_transpose2d(so5, weight_deconv5, stride=8) # dialtion so pool4 stride=1 so change stride from 16 to  8
        ### center crop
...
class FlipConv2d(_ConvNd):
    """
    Note: there may be minor problem with the bias term
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        super(FlipConv2d, self).__init__(in_channels, int(out_channels / 2), kernel_size, stride, padding, dilation,
         False, _pair(0), groups, bias)

    def forward(self, input):
        head = F.conv2d(input, self.weight, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        tail = F.conv2d(input, torch.flip(self.weight, dims=[3]), self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        return torch.cat((head, tail), dim=1)

class FlipConv2dLR(_ConvNd):
    """
    Flip Convolution (left-right flip)
    Note: there may be minor problem with the bias term
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True):
        ...
        dilation = _pair(dilation)
        assert out_channels % 2 == 0 and out_channels > 0
        super(FlipConv2dLR, self).__init__(
            in_channels, int(out_channels / 2), kernel_size, stride, padding, dilation,
            False, _pair(0), groups, bias)
...
class FlipConv2dUD(_ConvNd):
    """
    FlipConv2d (Upside Down flip)
    """
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        dilation = _pair(dilation)
        assert out_channels % 2 == 0 and out_channels > 0
        super(FlipConv2dUD, self).__init__(
            in_channels, int(out_channels / 2), kernel_size, stride, padding, dilation,
            False, _pair(0), groups, bias)
    ...
    def forward(self, input):
        head = F.conv2d(input, self.weight, self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        tail = F.conv2d(input, torch.flip(self.weight, dims=[2]), self.bias, self.stride,
                        self.padding, self.dilation, self.groups)
        return torch.cat((head, tail), dim=1)
